...
export default class Face2Scene extends FaceBase {
  ...
  create() {
    ...
    const label = this.add.text(this.scale.width / 2, 20, this.scene.key, {
      fontFamily: "Arial",
      fontSize: "28px",
      color: "#ffffff",
      })
    ...
    // same single variable to tune both hitbox + highlight (keep same scale as FaceTop for parity)
    ...
    const faceTravelTargets = [
      "FaceTopScene", // edge 0 -> back to FaceTopScene
      "Face3Scene",
      "Face4Scene",
      "Face5Scene",
      "Face6Scene",
    ...
    //this.addGrassyGroundTexture(width / 2, width / 2, radius);
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      gfx.fillRoundedRect(-hitWidth / 2, -hitHeight / 2, hitWidth, hitHeight, Math.max(6, Math.round(hitHeight / 2)));
      gfx.strokeRoundedRect(-hitWidth / 2, -hitHeight / 2, hitWidth, hitHeight, Math.max(6, Math.round(hitHeight / 2)));
      gfx.setPosition(e.mid.x, e.mid.y);
      gfx.setRotation(Phaser.Math.Angle.Between(e.start.x, e.start.y, e.end.x, e.end.y));
  ...
  private getEdges() {
    ...
    const edges: { start: Phaser.Math.Vector2; end: Phaser.Math.Vector2; mid: Phaser.Math.Vector2; length: number }[] = [];
  ...
  update(_time: number, delta: number) {
    ...
    for (const ez of this.edgeZones) {
      if (this.physics.world.overlap(this.player, ez.zone)) {
        ...
        ez.gfx.fillRoundedRect(-ez.width / 2, -ez.height / 2, ez.width, ez.height, Math.max(6, Math.round(ez.height / 2)));
        ez.gfx.strokeRoundedRect(-ez.width / 2, -ez.height / 2, ez.width, ez.height, Math.max(6, Math.round(ez.height / 2)));
        break;
      } else {
        ez.gfx.clear();
